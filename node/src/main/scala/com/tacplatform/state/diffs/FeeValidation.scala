...
object FeeValidation {
  ...
  val FeeConstants: Map[Byte, Long] = Map(
    ...
    IssueTransaction.typeId           -> 10000000,
    ReissueTransaction.typeId         -> 50000000,
    ...
    ExchangeTransaction.typeId        -> 10000,
    CreateAliasTransaction.typeId     -> 50000000,
      ...
      .map { baseFee =>
        tx match {
          ...
          case itx: IssueTransaction =>
            val multiplier = if (blockchain.isNFT(itx)) NFTMultiplier else 500
